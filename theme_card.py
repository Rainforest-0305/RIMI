...
def _subtitle_for(universe):
    ...
    ETF 프록시(대표지수) 경로 · 폴백 경로(대표종목 거래대금 가중) · 업종지수
    경로를 구분해 정직한 부제를 돌려준다. ETF 경로는 지수 실측이 아니라 추종
    ETF 종가 프록시임을 사실 라벨로 밝힌다. 어떤 판단/권유 어휘도 없다(사실만)."""
    u = str(universe or "")
    if "ETF 종가 프록시" in u:
        # 지수 실측이 아니라 추종 ETF(KODEX200/코스닥150) 종가 기반 프록시임을 명시
        return "코스피200·코스닥150 ETF 종가 프록시"
