...
TAG_RULES = [
    ...
    # [25] bare '분할' 은 주식분할·액면분할·'주식의 병합/분할'(전자등록 변경, 감자성)
    #   까지 과매칭 → M&A 성 분할만 매칭하도록 특정화(회사분할/분할합병/물적·인적분할).
    #   '합병' 은 '병합'(주식병합)과 글자순서가 달라 오매칭 안 함(합≠병합).
    ("합병분할",    ["합병", "회사분할", "분할합병", "물적분할", "인적분할",
                    "주식교환", "영업양수도", "자산양수도"]),
    ("공급계약",    ["공급계약", "수주", "단일판매"]),
    ("소송",        ["소송", "가처분", "회생", "파산"]),
    ("감사보고서",  ["감사보고서", "감사의견"]),
    ...
    for name, kws in TAG_RULES:
        for kw in kws:
            if kw.replace(" ", "") in title:
                tags.append(name)
                break
    # [25] '주권매매거래정지'는 M&A 결정이 아니라 절차적 거래정지 안내다(주식 병합·
    #   분할·전자등록 변경 등으로도 발생). 강한 M&A 키워드가 함께 없으면 합병분할 제외.
    if "합병분할" in tags and "매매거래정지" in title:
        _STRONG_MA = ("회사합병", "합병결정", "합병등", "분할합병", "회사분할",
                      "물적분할", "인적분할", "주식교환", "영업양수도", "자산양수도")
        if not any(k in title for k in _STRONG_MA):
            tags = [t for t in tags if t != "합병분할"]
    return tags or ["기타공시"]
