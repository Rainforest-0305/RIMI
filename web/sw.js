...
const CACHE = 'miri-v31';   // v30→v31: Google Ads 전환태그(gtag.js, AW-18355483264) 를 index.html·privacy.html 의 </head> 직전에 삽입. Google Ads 가 "사이트에서 Google 태그를 찾지 못했다"고 통지 → 전환측정·캠페인 최적화 불가 상태였음. 스니펫은 Google 제공 원문 그대로(async 유지, AdSense 지연로딩 블록은 무접촉). index.html 이 SHELL precache 라 bump 없으면 기존 설치 유저에게 구셸이 고착돼 태그가 영영 안 내려간다 → bump 필수
