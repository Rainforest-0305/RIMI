...
const CACHE = 'miri-v48';   // v47→v48: President 지시(2026-08-16) — 「레포트별로 나타나게 해줘」.
                            //   애널리스트 컨센서스를 «평균 하나»가 아니라 «어느 증권사가 언제 얼마를 냈는지»로 표시.
                            //   ①증권사 리포트 목록을 상시 노출(구동작: '자세히 보기' 뒤에 접혀 있어 아무도 못 봤다).
                            //     최신순 · 발표일 · 증권사 «전체 이름» · 투자의견 · 목표주가.
                            //   ②★증권사가 «1곳»이면 「평균 목표가」라는 말을 쓰지 않는다 — 지표 라벨·차트 점선
                            //     라벨·범례를 전부 그 증권사 이름으로 바꾼다(코웨이: 리포트 13건인데 증권사는
                            //     한화투자 «1곳». '평균'이라 부르면 사용자는 여러 곳의 컨센서스로 «믿는다»).
                            //   ③같은 증권사 중복은 행에 'N건' 표식(avg_tp 는 건수 가중이라 2건 낸 곳이 2표).
                            //   ④요약줄에 「증권사 N곳 · 리포트 M건 중 목표주가 P건」 상시 병기.
                            //   API 는 n_brokers·brokers 추가만(기존 avg_tp·n_total·n_tp 의미 «불변»).
                            //   index.html·shell.js 가 SHELL precache 라 bump 필수.
                            // (이전) v46→v47: President 지시 2건(2026-08-14) — 「관심 탭 대푯값 선택도 설정으로 몰아 / 맞춤법 전수 수정해」.
