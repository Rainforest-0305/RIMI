...
const CACHE = 'miri-v34';   // v33→v34: 설치 PWA(standalone)에서 로딩 화면을 «하나»로 — President 지시(2026-08-11). 기존엔 파란 부트로더(#bootloader, z300) 페이드아웃 «뒤» 스플래시(#splash, z200, 아이콘+이메일+가짜 진행바)가 또 떠서 로딩이 두 번 보였다. standalone 이면 #splash 를 즉시 remove 하고, 완료 판정(.card 1장 존재 · 최소 0.9s · 상한 6s)을 부트로더가 «그대로 승계»한다 — load 직후 숨기면 데이터 도착 전이라 빈 화면이 잠깐 보이기 때문. 일반 브라우저는 #bootloader 가 display:none 이라 스플래시가 유일한 로딩 화면으로 남고 «동작 변화 없음». index.html 이 SHELL precache 라 bump 없으면 기존 설치 유저에게 구셸이 고착된다 → bump 필수
